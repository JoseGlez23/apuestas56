...
export const AuthProvider = ({ children }) => {
  ...
  const [adminUser, setAdminUser] = useState(null); // Nueva variable de estado para el administrador
  ...
  const login = async (email, password) => {
    try {
      ...
      if (userCredential.user.email !== 'admin@gmail.com') {
        ...
        setAdminUser(null); // No es administrador
      }
    } catch (error) {
      setGlobalMsg(error.message);
    }
  ...
  const adminLogin = async (email, password) => {
    if (email === 'admin@gmail.com' && password === 'adminpro') {
      try {
        ...
        setCurrentUser(null); // Es administrador
      } catch (error) {
        setGlobalMsg(error.message);
      }
    } else {
  ...
  const logout = async () => {
    try {
      ...
      setCurrentUser(null);
      setAdminUser(null); // Restablecer ambos estados
    } catch (error) {
      setGlobalMsg(error.message);
    }
  };
  ...
  return (
    <AuthContext.Provider
      value={{
        currentUser,
        adminUser, // Añadido el usuario administrador al contexto
        register,
        login,
        adminLogin, // Añadido el login de administrador al contexto
        logout,
        bidAuction,
        endAuction,
        globalMsg,
